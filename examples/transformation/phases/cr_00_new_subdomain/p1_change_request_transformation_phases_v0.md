...
from; P1 classifies and traces, it does not add.
...
| Classification | Rationale | Source Finding |
|---|---|---|
| NEW_SUBDOMAIN | The pipeline that decides which changes are admissible is a distinct concern from the capabilities it admits and needs its own governance boundary. It extends nothing that exists. | CR seed §1 CR Type |

## 2. Business Vocabulary

| Term | Definition | Source Finding |
|---|---|---|
| Phase | One governed step of the change pipeline, with declared inputs, outputs and rules. | CR seed §2 Business Vocabulary |
| Seed | The business problem statement reorganized into fixed registers, and the only input later phases accept. | CR seed §2 Business Vocabulary |
...
| Fact | Certainty | Source Finding |
|---|---|---|
| The change pipeline must be governed the same way the capabilities it admits are governed. | HIGH | CR seed §4 Known Facts #1 |
...
| The rule set is governance and must be readable from the composition and versioned as declared behavior. | HIGH | CR seed §4 Known Facts #5 |
...
| Changing the rule set requires the same governed change process as any other declared behavior. | HIGH | CR seed §4 Known Facts #12 |
...
|---|---|
| A seed has exactly one verdict. | CR seed §8 Invariants #1 |
...
| Scope Item | Relationship | Source Finding |
|---|---|---|
| phases | CREATED | CR seed §13 Governance Scope |
...
Both clarifications the seed carried have been answered by the human and are recorded here rather
than resolved by the phase.

1. *How is the rule set carried as declared data?* Answered: it travels as declared node input in
   the phase's own workflow, where it is compiled, sealed and readable from the composition. Belief
   6 was verified in the course of answering it — no existing form declares rules applied to a
   document rather than to artifacts.
2. *Does the phase receive the seed as text, or as a location it must read?* Answered: as text. The
   driver reads the file; nothing downstream touches the filesystem, which is what makes a verdict
   reproducible.

...
| Q3 | The capability now exists and the platform holds it. An act announces an ordered sequence at one ending, the order is normative, a moment announced twice is refused, and an announcement that cannot be made is reported rather than dropped. | Removes the blocker this dossier halted on, and fixes what the design may now state. | OBSERVED | HIGH | CLOSED | `fb.workflow::CONSTITUTION_WORKFLOW_V0` §2a states the model; `fb.workflow::INVARIANT_WF_ANNOUNCEMENT_DISTINCT_V0` holds it; the composition seals a sequence per transition and the runtime announces each in the sealed order. |

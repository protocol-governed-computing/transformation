...
> Every register below is present and correctly shaped. Each is missing the one cell that carries the business knowledge the phase exists to state.
...
|--------|----------------------------------|------------|
| CR seed §0 Subdomain Purpose | REFINED |  |
<!-- was: States the authority the subdomain establishes — a book exists in the collection because the catalog says so — the lifecycle it manages from registration through retirement and back, and the four functions it explicitly does not govern. The seed states what the catalog is for and why it exists; none of these four additions contradicts it. |
...
| Catalog audit trail | APPEND_ONLY | Every operation must be traceable afterwards, and a trail that could be amended would not be evidence | S4 resources Catalog audit trail |
...
|------------|----------------|--------|-----------------|------------------------------|----------------|
| Book record | Title, author and publication year together | Supplied by the staff member registering the book |  | None | S1 identity_and_sameness #1 |
...
| A book carries at least one subject |  | S1 business_invariants #5 |

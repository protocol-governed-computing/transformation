...
| Enforce that one physical copy exists per barcode | REUSE | A barcode is a single value, so the registry's own key is the business key with no convention added. | transformation::CT_PURE_PARSE_REGISTERS_V0 and workload::EV_CONJECTURE_EVALUATED_V0 were offered as candidates. | S3 analysis_findings #2 |

...
> Every register below is present and correctly shaped. Each is missing the one cell that carries the decision the phase exists to record, or hedges it.
...
| Register a further physical copy against a registered book | catalog | SHARED |  | S4 authoring_scope GAP-06 |
...
| Claim a value once so a second claim on it fails | platform | SATISFIED |  | S3 authoring_decisions Enforce that one book exists per title, author and publication year |
| Append an entry to a trail that cannot be amended | platform | UNRESOLVED | capability_side_effects::CS_APPENDONLY_JSONL_V0 | S3 authoring_decisions Append an entry to an append-only trail |
...
|------------|-----------|-------------------|-------------------------|----------------|
| Read whether a staff member is authorized to perform catalog operations | catalog → staff |  | SATISFIED | S1 authority_deferrals #1 |

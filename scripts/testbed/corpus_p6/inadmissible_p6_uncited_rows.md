...
> Every row below places a capability or names an artifact. What is wrong with each is where it says it came from, or whether the thing it names is there.
...
| Hold a durable record that can be read, listed and updated in place | platform | SATISFIED | catalog::CS_MUTABLE_JSON_V0 | S3 authoring_decisions Hold a book record durably and update it in place |
...
|--------------|---------|-----------|----------------|
| A durable record of every book the library catalogs | The library requires one authoritative description per book, correctable in place, carrying its own registered-or-retired state | catalog | S0 business_invariants #99 |
...
| ai_governance::CS_REGISTRY_V0 | Declared and in use by ai_governance | REUSE | S3 impact_analysis capability_side_effects::CS_REGISTRY_V0 |
...
|-----------|-----------|----------------|
| CATALOG_OWNS_ITS_STORES | Every store the catalog reads or writes is declared by the catalog, and no catalog operation writes into a store another subdomain owns. |  |
...
|------------|-----------------|----------------|
| Register a book together with its first physical copy | catalog | S6 placement_register Register a book |

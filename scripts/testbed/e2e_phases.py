...
# phase, workflow, payload file, expected verdict, expected finding count, expected rules evaluated.
...
CASES = [
    ("P0", "transformation::WF_P0_SEED_ADMISSIBILITY_V0",
     "01_admissible_seed.json", "ADMISSIBLE", 0, 75),
    ("P0", "transformation::WF_P0_SEED_ADMISSIBILITY_V0",
     "02_admissible_reference.json", "ADMISSIBLE", 0, 75),
    ("P0", "transformation::WF_P0_SEED_ADMISSIBILITY_V0",
     "03_inadmissible_seven_violations.json", "INADMISSIBLE", 7, 75),
    ("P0", "transformation::WF_P0_SEED_ADMISSIBILITY_V0",
     "04_inadmissible_structural.json", "INADMISSIBLE", 2, 75),
    ("P0", "transformation::WF_P0_SEED_ADMISSIBILITY_V0",
     "05_inadmissible_truncated.json", "INADMISSIBLE", 6, 75),
    ("P1", "transformation::WF_P1_CHANGE_REQUEST_ADMISSIBILITY_V0",
     "06_p1_admissible_register.json", "ADMISSIBLE", 0, 109),
    ("P1", "transformation::WF_P1_CHANGE_REQUEST_ADMISSIBILITY_V0",
     "07_p1_inadmissible_register.json", "INADMISSIBLE", 5, 109),
]
...
def main() -> int:
    ...
    failures = 0
    for phase, wf, payload_file, want_verdict, want_findings, want_rules in CASES:
        ...
        findings = len(result.surface.get("findings") or [])
        ...
        if findings != want_findings:
            problems.append(f"findings {findings} != {want_findings}")
        if rules != want_rules:
            problems.append(f"rules {rules} != {want_rules}")

        mark = "OK   " if not problems else "FAIL "
        print(
            f"  {mark}  {phase}  {payload_file:<38} "
            f"{str(verdict):<13} {findings:>2} finding(s)  {rules} rules"
        )
        if problems:
            failures += 1

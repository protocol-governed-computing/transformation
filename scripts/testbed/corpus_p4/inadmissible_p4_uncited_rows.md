...
> P4 consolidates Stages 1 to 3 and introduces nothing. Every row below is a faithful consolidation. What is wrong with each is where it says it was consolidated from.
...
|-------|------|-----------------|----------------|
| Authorized staff member | Performs every catalog operation, and judges when a record is obsolete | Operator |  |
...
|----------|-------------|----------------|
| Book records | The catalog's authoritative description of every book, held by CS_MUTABLE_JSON_V0 | S2 resource_register Book |
...
|---|-----------|----------------|--------|
| 1 | Each physical copy belongs to exactly one book. | S3 analysis_findings #99 | invariant |

...
> Every register below is present and correctly shaped. Each records a consolidation with the cell that traces it to evidence left empty, or hedged.
...
|--------|-------------|-------------|----------------|
| Book | UNRESOLVED | One durable record per book, addressed by title, author and publication year together, updatable in place, carrying its own state | S2 entities Book |
...
| Record a performed catalog operation in the catalog's audit trail | S3 authoring_decisions Record a performed catalog operation in the catalog's audit trail | CRITICAL |  | Nothing in the composition satisfies it. |
...
| 2 | The catalog owns its audit composition and its own append-only audit store, reusing only the append-only mechanism beneath them. | S3 analysis_findings #1 |  | Two artifacts are authored rather than one reused; no catalog operation writes into another subdomain's store. |
...
| Declare the stores the catalog owns |  |

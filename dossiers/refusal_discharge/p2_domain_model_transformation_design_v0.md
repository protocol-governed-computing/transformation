...
| Refuses an obligation nothing is bound to | governance::INVARIANT_GOVERNANCE_DECLARATION_RESOLVES_V0 | Fails the build where a declaration resolves to nothing. | EXACT | Nothing — it is the same closure one layer down, and the argument this change borrows. |
